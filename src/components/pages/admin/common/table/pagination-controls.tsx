...
> = ({ table, isLoadingNextPage }) => {
  const id = useId();
  const pageNumberInputId = `${id}-pageNumber`;
  const pageSizeSelectId = `${id}-pageSize`;
  ...
  return (
    <div className="mt-5 mb-5 flex flex-row flex-wrap items-center justify-center gap-4 sm:mt-10">
      ...
      <span className="mx-5">
        <label htmlFor={pageNumberInputId} className="mr-2">
          Mergi la pagina:
        </label>{" "}
        <input
      ...
      <span className="mx-5">
        <label htmlFor={pageSizeSelectId} className="mr-2">
          Afișează:
        </label>
        <select
          id={pageSizeSelectId}
          ...
          className="rounded-lg bg-slate-800 px-2 py-1 text-white hover:cursor-pointer"
        >
